...
let seconds = 0;
let ur = undefined;
let ur_answer = undefined;
let selected_cell = -1;

const getGameInfo = () => JSON.parse(localStorage.getItem('game'));
// agrego espacio para 9 celdas
const initGameGrid = () => {
    let index = 0;
...
const initSudoku = () => {
    ...
    for (let i = 0; i < Math.pow(CONSTANT.GRID_SIZE, 2); i++) {
        ...
        cells[i].setAttribute('data-value', ur.question[row][col]);

        if (ur.question[row][col] !== 0) {
            cells[i].classList.add('filled');
            cells[i].innerHTML = ur.question[row][col];
        }
    }
}
const loadSudoku = () => {
    ...
    for (let i = 0; i < Math.pow(CONSTANT.GRID_SIZE, 2); i++) {
        let row = Math.floor(i / CONSTANT.GRID_SIZE);
        let col = i % CONSTANT.GRID_SIZE;
        cells[i].setAttribute('data-value', ur_answer[row][col]);
        cells[i].innerHTML = ur_answer[row][col] !== 0 ? ur_answer[row][col] : '';
        if (ur.question[row][col] !== 0) {
            cells[i].classList.add('filled');
        }
    }
}
//muestro los casilleros en la misma col y fila de la celda elegida
const hoverBg = (index) => {
...
const isGameWin = () => sudokuCheck(ur_answer);
const showResult = () => {
    clearInterval(timer);
